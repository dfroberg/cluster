...
resource "xenorchestra_vm" "kube-workers" {
  for_each = var.workers
  ...
  auto_poweron = true
  cloud_config = xenorchestra_cloud_config.worker_user_config[each.key].template
  cloud_network_config = xenorchestra_cloud_config.worker_network_config[each.key].template
  name_label = each.key
  name_description = "k3s worker node CIDR: ${each.value.cidr}"
