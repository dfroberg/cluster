...
resource "xenorchestra_vm" "kube-storage" {
  for_each = var.storage
  ...
  auto_poweron = true
  cloud_config = xenorchestra_cloud_config.storage_user_config[each.key].template
  cloud_network_config = xenorchestra_cloud_config.storage_network_config[each.key].template
  name_label = each.key
  name_description = "k3s storage node CIDR: ${each.value.cidr}"
